...
)
{
	...
	for (;;)
        {
		wch = p[si++];					/* Get a character */
		if (wch < ' ' || wch == '/' || wch == '\\') break;	/* Break on end of segment */
		...
		if (IsDBCS1(wch))
			...
			wch = ((wch << 8) + b);			/* Create a DBC */
		}
		wch = ff_convert(wch, 1);			/* Convert ANSI/OEM to Unicode */
		if (!wch) return FR_INVALID_NAME;	/* Reject invalid code */
#endif
		if (wch < 0x80 && chk_chr("\"*:<>\?|\x7F", wch)) /* Reject illegal characters for LFN */
			return FR_INVALID_NAME;
		lfn[di++] = wch;					/* Store the Unicode character */
	}
	*path = &p[si];						/* Return pointer to the next segment */
	cf = (wch < ' ') ? NS_LAST : 0;		/* Set last segment flag if end of path */
#if _FS_RPATH
	...
	b = i = 0; ni = 8;
	for (;;) {
		wch = lfn[si++];					/* Get an LFN character */
		if (!wch) break;					/* Break on end of the LFN */
		if (wch == ' ' || (wch == '.' && si != di))
		...
		if (wch >= 0x80)
                {				/* Non ASCII character */
#ifdef _EXCVT
			wch = ff_convert(wch, 0);		/* Unicode -> OEM code */
			if (wch) wch = ExCvt[wch - 0x80];	/* Convert extended character to upper (SBCS) */
#else
	...
	*p = 0;		/* Terminate SFN string by a \0 */
	...
	if (fno->lfname)
		...
		if (dp->sect && fno->lfsize && dp->lfn_idx != 0xFFFF) /* Get LFN if available */
                {
			lfn = dp->lfn;
			while ((wch = *lfn++) != 0)		/* Get an LFN character */
                        {
#if !_LFN_UNICODE
                                wch = ff_convert(wch, 0);	/* Unicode -> OEM */
				if (!wch)                       /* No LFN if it could not be converted */
                                {
                                  i = 0;
                                  break;
                                }
				if (_DF1S && wch >= 0x100)	/* Put 1st byte if it is a DBC (always false on SBCS cfg) */
                                {
				  p[i++] = (TCHAR)(wch >> 8);
                                }
#endif /* _LFN_UNCODE */
