...
__ALIGN_BEGIN uint8_t usbd_cdc_CfgDesc  [USB_CDC_VCP_CONFIG_DESC_SIZ] __ALIGN_END ;
...
#endif 
        len = MIN(USB_CDC_VCP_DESC_SIZ , req->wLength);
      }
...
static uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t epnum)
{
	...
	if (USB_Tx_length) {
		USB_Tx_length = MIN_(USB_Tx_length, CDC_VCP_DATA_IN_PACKET_SIZE);
	} else if (last_tx_packet_size(pdev, epnum) != CDC_VCP_DATA_IN_PACKET_SIZE) {
		USB_Tx_Active = 0;
		return USBD_OK;
	}
...
static inline int try_start_rx(void *pdev)
{
	...
	if (space_avail < CDC_VCP_DATA_OUT_PACKET_SIZE) {
