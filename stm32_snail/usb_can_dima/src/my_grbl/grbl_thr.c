...
for (;;) 
  {
    while((cc = serial_read_rx()) != SERIAL_NO_DATA) {
      if ((cc == '\n') || (cc == '\r')) { // End of line reached
        ...
        buf_line[char_counter] = 0; // Set string termination character.
        ...
        } else if (buf_line[0] == 0) {
          // Empty or comment line. For syncing purposes.
          report_status_message(STATUS_OK);
        } else if (buf_line[0] == '$') {
          // Grbl '$' system command
          report_status_message(system_execute_line(buf_line));
        } else if (sys.state & (STATE_ALARM | STATE_JOG)) {
          // Everything else is gcode. Block if in alarm or jog mode.
          report_status_message(STATUS_SYSTEM_GC_LOCK);
        } else {
          // Parse and execute g-code block.
          report_status_message(gc_execute_line(buf_line));
        }
        ...
        if (line_flags) {
          ...
          } else if (cc >= 'a' && cc <= 'z') { // Upcase lowercase
            buf_line[char_counter++] = cc-'a'+'A';
          } else {
            buf_line[char_counter++] = cc;
          }
        }
