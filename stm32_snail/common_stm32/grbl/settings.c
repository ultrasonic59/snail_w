...
{
	uint8_t buf[sizeof(settings_t) + 1];

	buf[0] = SETTINGS_VERSION;
	memcpy(buf + 1, (char*)&settings, sizeof(settings_t));
...
void settings_restore(uint8_t restore_flag) {
  ...
  if (restore_flag & SETTINGS_RESTORE_DEFAULTS) {
	      ...
	      settings.status_report_mask = (uint8_t)DEFAULT_STATUS_REPORT_MASK;
	      ...
	      settings.homing_dir_mask = (uint8_t)DEFAULT_HOMING_DIR_MASK;
	      settings.homing_feed_rate = DEFAULT_HOMING_FEED_RATE;
	      settings.homing_seek_rate = DEFAULT_HOMING_SEEK_RATE;
	      settings.homing_debounce_delay = (uint16_t)DEFAULT_HOMING_DEBOUNCE_DELAY;
	      settings.homing_pulloff = DEFAULT_HOMING_PULLOFF;

	      settings.flags = 0;
	      if (DEFAULT_REPORT_INCHES) { settings.flags |= (uint8_t)BITFLAG_REPORT_INCHES; }
	      if (DEFAULT_LASER_MODE) { settings.flags |= (uint8_t)BITFLAG_LASER_MODE; }
	      if (DEFAULT_INVERT_ST_ENABLE) { settings.flags |= (uint8_t)BITFLAG_INVERT_ST_ENABLE; }
	      if (DEFAULT_HARD_LIMIT_ENABLE) { settings.flags |= (uint8_t)BITFLAG_HARD_LIMIT_ENABLE; }
	      if (DEFAULT_HOMING_ENABLE) { settings.flags |= (uint8_t)BITFLAG_HOMING_ENABLE; }
	      if (DEFAULT_SOFT_LIMIT_ENABLE) { settings.flags |= (uint8_t)BITFLAG_SOFT_LIMIT_ENABLE; }
	      if (DEFAULT_INVERT_LIMIT_PINS) { settings.flags |= (uint8_t)BITFLAG_INVERT_LIMIT_PINS; }
	      if (DEFAULT_INVERT_PROBE_PIN) { settings.flags |= (uint8_t)BITFLAG_INVERT_PROBE_PIN; }
...
uint8_t settings_store_global_setting(uint8_t parameter, float value) {
  ...
  if (parameter >= AXIS_SETTINGS_START_VAL) {
    ...
    uint8_t int_value = (uint8_t)trunc(value);
    switch(parameter) {
      case 0:
