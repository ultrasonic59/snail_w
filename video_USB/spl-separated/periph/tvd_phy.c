...
void tvd_config(unsigned int interface, unsigned int system, unsigned int format)
{
	...
	{ /* composite */
		...
		case TVD_SOURCE_NTSC:
			...
			TVD_RegWrite(TVD_REG_BASE + 0x000c,
			...
			TVD_RegWrite(TVD_REG_BASE + 0x001c,
				 0x00820022); /* hactive and vactive start */
			TVD_RegWrite(TVD_REG_BASE + 0x0f08,
				 0x00590100); /* notch width 0059_0100 */
			TVD_RegWrite(TVD_REG_BASE + 0x0f0c,
				 0x00000010); /* YC sep */
			TVD_RegWrite(TVD_REG_BASE + 0x0f10,
				 0x008A32DD); /* sync height */
			TVD_RegWrite(TVD_REG_BASE + 0x0f14, 0x80000080); /* */
			/*?		TVD_RegWrite(TVD_REG_BASE+0x0018, */
			/*0x00002080); //adjust luma brightness */
			TVD_RegWrite(TVD_REG_BASE + 0x0f1c,
				 0x008A0000); /* chroma AGC target */
			TVD_RegWrite(TVD_REG_BASE + 0x0f2c, 0x0000CB74);
			TVD_RegWrite(TVD_REG_BASE + 0x0f44,
				 0x00004632); /* burst gate */
			TVD_RegWrite(TVD_REG_BASE + 0x0f74,
				 0x000003c3); /* chroma edge enhance */
			/*		TVD_RegWrite(TVD_REG_BASE+0x0f74, */
			/*0x00000340); //default */

			TVD_RegWrite(TVD_REG_BASE + 0x0f80,
				 0x00500000); /* hactive width */
			TVD_RegWrite(TVD_REG_BASE + 0x0f84,
				 0x00610000); /* vactive height */
			TVD_RegWrite(TVD_REG_BASE + 0x0000,
		...
		case TVD_SOURCE_PAL:
			...
			TVD_RegWrite(TVD_REG_BASE + 0x0f14,
			...
			/* TVD_RegWrite(TVD_REG_BASE+0x001c, 0x00870056); //hactive
			 */
			/* and vactive start */

			/* TVD_RegWrite(TVD_REG_BASE+0x0e04, 0x00000000); */
			if (TVD_MB_YUV420 == format) {
				tvd_set_width(0, 704);
				tvd_set_width_jump(0, 704);
				tvd_set_height(0, 224);
		  ...
		  /* TVD_RegWrite(TVD_REG_BASE+0x0504,0x00000000); */
		  /* TVD_RegWrite(TVD_REG_BASE+0x052c,0x00110000); */
		  /* 1 channel cvbs */
		  /* TVD_RegWrite(TVD_REG_BASE+0x0500,0x00000111); */
		  /* TVD_RegWrite(TVD_REG_BASE+0x0000,0x00000321); */
		  /* default open all 4 channels if you don't care power */
		  /* consumption */
		  /* TVD_RegWrite(TVD_REG_BASE+0x0500,0x00000f11); */

		/*		TVD_RegWrite(TVD_REG_BASE+0x0e2c,0x60000000); */
	}
#if 1
	{
		__u32 v;
...
void tvd_set_other(int d,int y)
{
	unsigned int tmp;
	if(d)
	{
		TVD_RegWrite(TVD_REG_BASE+0x004c,y);
		TVD_RegWrite(TVD_REG_BASE+0x0050,y+0x00200000);
...
void tvd_irq_enable(unsigned int id,tvd_irq_t irq)
{
	unsigned int reg_val;
	switch(irq){
		case TVD_FRAME_DONE:
...
void tvd_irq_disable(unsigned int id,tvd_irq_t irq)
{
	unsigned int reg_val;
	switch(irq)
	{
...
void tvd_irq_status_clear(unsigned int id,tvd_irq_t irq)
{
	unsigned int reg_val;
	switch(irq){
		case TVD_FRAME_DONE:
...
void tvd_bsp_init(int id)
{
	...
	TVD_RegWrite(TVD_REG_BASE + 0x0e2c, 0x000b0000);
	...
	TVD_RegWrite(TVD_REG_BASE + 0x0000, 0x00001f01);
