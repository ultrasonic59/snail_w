...
static int tvd_id = 0;
unsigned char tvd_y_buf[CAMERA_W*CAMERA_H];
unsigned char tvd_c_buf[CAMERA_W*CAMERA_H];

unsigned char tvd_y_buf_disp[DISPLAY_W*DISPLAY_H];
unsigned char tvd_c_buf_disp[DISPLAY_W*DISPLAY_H];
...
//				tvd_y_buf[1004],tvd_y_buf[1005],tvd_y_buf[1006],tvd_y_buf[1007]);
	//tcountt ++;
////	memcpy(tvd_y_buf_disp,tvd_y_buf,CAMERA_W*CAMERA_H);
/////	memcpy(tvd_c_buf_disp,tvd_c_buf,CAMERA_W*CAMERA_H);
	tvd_irq_status_clear(tvd_id,TVD_FRAME_DONE);

	// dma_cache_sync(tvd_y_buf,720*576,3);
	// dma_cache_sync(tvd_c_buf,720*576,3);
	RefleshLcdWithTVD(tvd_y_buf_disp,tvd_c_buf_disp,CAMERA_W,CAMERA_H);
...
void tv_in_decode_init(int id)
{
	...
	tvd_config(TVD_SOURCE_CVBS,TVD_SOURCE_PAL,TVD_PL_YUV422);
	TVD_Printf("tvd_config \n");

	tvd_set_fmt(id,TVD_PL_YUV422);
