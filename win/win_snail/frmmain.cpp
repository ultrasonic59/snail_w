...
{
    return 0;////???   return ui->grpConsole->height() - ui->grpConsole->contentsRect().height() + ui->spacerConsole->geometry().height() + ui->grpConsole->layout()->margin() * 2 + ui->cboCommand->height();
}
...
{
    return true;////??? ui->txtMPosX->text().toDouble() == x && ui->txtMPosY->text().toDouble() == y && ui->txtMPosZ->text().toDouble() == z;
}
...
void frmMain::setCamera(const QByteArray& cameraDevice)
{
    delete camera;
