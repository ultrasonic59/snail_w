    ...
    while(SerialIf_CanReadLine())
    {
        ...
        if(data[0] == '<')
        {
            ...
            if (stx.indexIn(data) != -1)
            {
				...
				ui->cmdSetOrigin->setEnabled(status == IDLE || status == ALARM);
#ifdef WINDOWS
            ...
            {
                UpdateOverride(ui->slbFeedOverride, ov.cap(1).toInt(), static_cast<const unsigned char>(0x91));
                UpdateOverride(ui->slbSpindleOverride, ov.cap(3).toInt(), static_cast<const unsigned char>(0x9a));
        ...
        else if(data.length() > 0)
        {
            ...
            if(mCommandsSent.size() > 0 && !DataIsFloating(data) && !(mCommandsSent.front().command != "[CTRL+X]" && DataIsReset(data)))
            {
