...
void frmMain::updateControlsState()
{
	...
	ui->cmdSetOrigin->setEnabled(portOpened);
...
void frmMain::onActSendFromLineTriggered()
{
    ...
    if (m_settings->autoLine())
    {
		...
		if(lineNumber < 0)
		{
			QMessageBox::warning(0, tr("No command"), tr("Command %1 is not programmed, selet other line").arg(m_currentModel->data(m_currentModel->index(commandIndex, 1)).toString()));
			return;
		}
...
void frmMain::SetRefferenceAlign()
{
	ui->txtStartPosX->setText(QString::number(m_reference_align.x(), 'f', 3));
	ui->txtStartPosY->setText(QString::number(m_reference_align.y(), 'f', 3));
	ui->txtStartPosZ->setText(QString::number(m_reference_align.z(), 'f', 3));
}


void frmMain::ApplyRefferenceAlign()
{
	bool result = false;
	float value = ui->txtStartPosX->text().toFloat(&result);
	if(result)
		m_reference_align.setX(value);

	value = ui->txtStartPosY->text().toFloat(&result);
	if(result)
		m_reference_align.setY(value);

	value = ui->txtStartPosZ->text().toFloat(&result);
	if(result)
		m_reference_align.setZ(value);
}


void frmMain::SetRefferenceAlignMode()
{
	ui->radio_no_move->setChecked(refference_align_move == 0);
	ui->radio_to_XY->setChecked(refference_align_move == 1);
	ui->radio_to_XYZ->setChecked(refference_align_move == 2);
}

void frmMain::ApplyRefferenceAlignMode()
{
	if(ui->radio_no_move->isChecked())
		refference_align_move = 0;
	else
	{
		if(ui->radio_to_XY->isChecked())
			refference_align_move = 1;
		else
		{
			if(ui->radio_to_XYZ->isChecked())
				refference_align_move = 2;
		}
	}
}
...
void frmMain::on_cmdSetOrigin_clicked()
{
    m_homing = true;
    m_updateSpindleSpeed = true;
	...
	ApplyRefferenceAlign();
	ApplyRefferenceAlignMode();

	if(refference_align_move == 2)
	{
		sendCommand(QString("G53G90G0X%1Y%2Z%3").arg(m_reference_align.x(), 0,'f', 3)
												.arg(m_reference_align.y(), 0,'f', 3)
												.arg(m_reference_align.z(), 0,'f', 3), -1, m_settings->showUICommands());
		sendCommand("G92X0Y0Z0", -1, m_settings->showUICommands());
	}
	else
	{
		sendCommand(QString("G92X%1Y%2Z%3").arg(-m_reference_align.x(), 0,'f', 3)
										.arg(-m_reference_align.y(), 0,'f', 3)
										.arg(-m_reference_align.z(), 0,'f', 3), -1, m_settings->showUICommands());

		if(refference_align_move == 1)
