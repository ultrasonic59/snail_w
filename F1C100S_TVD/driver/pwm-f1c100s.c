...
void pwm_f1c100s_config_ext(pwm_t * pwm, int32_t duty, int32_t period, int32_t prescaler,int32_t polarity)
{
	uint64_t rate = 24000000;//hosc =24MHz //clk_get_rate(pwm->clk);
	uint32_t prd, dty, gate,ctrl;
	prd = period;
	dty = duty;
...
void pwm_f1c100s_set_duty_ext(pwm_t * pwm, int32_t duty)
{
	pwm_f1c100s_config_ext(pwm, duty, pwm->period,0x0f, pwm->polarity);
}
...
int32_t pwm_f1c100s_init_ext(pwm_t * pwm)
{
	write32(pwm->virt + PWM_CTRL, read32(pwm->virt + PWM_CTRL) &~(0x3fff << (pwm->channel * 15)));
	write32(pwm->virt + PWM_PERIOD(pwm->channel), 0);

	pwm_f1c100s_config_ext(pwm, pwm->duty, pwm->period,0x0f, pwm->polarity);
	pwm_f1c100s_enable(pwm);
	return 0;
}
