...
*/
    ...
    /* Configure USART1 Tx (PB15) as alternate function push-pull */
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_15;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    ...
    if( Uart.Rx_RemainLen )
    {
        if( Uart.USB_Up_IngFlag == 0 )
        {
            ...
            if( packlen )
            {
                ...
                Uart.USB_Up_TimeOut = 0x00;
                USBHS_Endp_DataUp( DEF_UEP2, &UART1_Rx_Buf[ Uart.Rx_DealPtr ], packlen, DEF_UEP_DMA_LOAD );
                NVIC_EnableIRQ( USBHS_IRQn );
    ...
    if( Uart.USB_Up_Pack0_Flag )
    {
        if( Uart.USB_Up_IngFlag == 0 )
        {
            if( Uart.USB_Up_TimeOut >= ( DEF_UARTx_RX_TIMEOUT * 20 ) )
            {
                ...
                Uart.USB_Up_IngFlag = 0x01;
                Uart.USB_Up_TimeOut = 0x00;
                USBHS_Endp_DataUp( DEF_UEP2, &UART1_Rx_Buf[ Uart.Rx_DealPtr ], 0, DEF_UEP_DMA_LOAD );
