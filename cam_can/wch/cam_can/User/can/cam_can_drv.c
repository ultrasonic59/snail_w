...
void CAN_wrMsg (can_msg_t *msg)  {
  ...
  CAN1->sTxMailBox[0].TXMIR = 0;           /* Reset TIR register               */
                                          /* Setup identifier information     */
  if (msg->format == STANDARD_FORMAT) {   /*    Standard ID                   */
    CAN1->sTxMailBox[0].TXMIR |= (uint32_t)(msg->id << 21) | CAN_ID_STD;
  } else {                                /* Extended ID                      */
    CAN1->sTxMailBox[0].TXMIR |= (uint32_t)(msg->id <<  3) | CAN_ID_EXT;
  }
                                          /* Setup type information           */
  if (msg->type == DATA_FRAME)  {         /* DATA FRAME                       */
    CAN1->sTxMailBox[0].TXMIR |= CAN_RTR_DATA;
  } else {                                /* REMOTE FRAME                     */
    CAN1->sTxMailBox[0].TXMIR |= CAN_RTR_REMOTE;
  }
                                          /* Setup data bytes                 */
  CAN1->sTxMailBox[0].TXMDLR = (((uint32_t)msg->data[3] << 24) |
                              ((uint32_t)msg->data[2] << 16) |
                              ((uint32_t)msg->data[1] <<  8) | 
                              ((uint32_t)msg->data[0])        );
  CAN1->sTxMailBox[0].TXMDHR = (((uint32_t)msg->data[7] << 24) |
                              ((uint32_t)msg->data[6] << 16) |
                              ((uint32_t)msg->data[5] <<  8) |
                              ((uint32_t)msg->data[4])        );
                                          /* Setup length                     */
  CAN1->sTxMailBox[0].TXMDTR &= ~CAN_TDT0R_DLC;
  CAN1->sTxMailBox[0].TXMDTR |=  (msg->len & CAN_TDT0R_DLC);

  CAN1->INTENR |= CAN_INTENR_TMEIE;                 /* enable  TME interrupt        */
  CAN1->sTxMailBox[0].TXMIR |=  TMIDxR_TXRQ;  /* transmit message             */
}
...
void CAN_rdMsg (can_msg_t *msg)  {
  ...
  if ((CAN1->sFIFOMailBox[0].RXMIR & CAN_ID_EXT) == 0) {
    msg->format = STANDARD_FORMAT;
    msg->id     = 0x000007FF & (CAN1->sFIFOMailBox[0].RXMIR >> 21);
  } else {
    msg->format = EXTENDED_FORMAT;
    msg->id     = 0x1FFFFFFF & (CAN1->sFIFOMailBox[0].RXMIR >> 3);
  }
                                              /* Read type information        */
  if ((CAN1->sFIFOMailBox[0].RXMIR & CAN_RTR_REMOTE) == 0) {
    msg->type =   DATA_FRAME;
  } else {
    msg->type = REMOTE_FRAME;
  }
                                              /* Read number of rec. bytes    */
  msg->len     = (CAN1->sFIFOMailBox[0].RXMDTR      ) & 0x0F;
                                              /* Read data bytes              */
  msg->data[0] = (CAN1->sFIFOMailBox[0].RXMDLR      ) & 0xFF;
  msg->data[1] = (CAN1->sFIFOMailBox[0].RXMDLR >>  8) & 0xFF;
  msg->data[2] = (CAN1->sFIFOMailBox[0].RXMDLR >> 16) & 0xFF;
  msg->data[3] = (CAN1->sFIFOMailBox[0].RXMDLR >> 24) & 0xFF;

  msg->data[4] = (CAN1->sFIFOMailBox[0].RXMDHR      ) & 0xFF;
  msg->data[5] = (CAN1->sFIFOMailBox[0].RXMDHR >>  8) & 0xFF;
  msg->data[6] = (CAN1->sFIFOMailBox[0].RXMDHR >> 16) & 0xFF;
  msg->data[7] = (CAN1->sFIFOMailBox[0].RXMDHR >> 24) & 0xFF;

  CAN1->RFIFO0 |= CAN_RFIFO0_RFOM0;             /* Release FIFO 0 output mailbox */
}

void CAN1_RX0_IRQHandler (void)
{
if (CAN1->RFIFO0 & CAN_RFIFO0_FMP0)
  {			/* message pending ?              */
...
{
if (CAN1->RFIFO1 & CAN_RFIFO1_FMP1)
  {			/* message pending ?              */
  CAN_rdMsg (&CAN_RxMsg);                 /* read the message               */
  CAN_RxRdy = 1;                          // set receive flag
  }
