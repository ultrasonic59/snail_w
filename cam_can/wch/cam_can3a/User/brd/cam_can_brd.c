...
//=========== tim_pwm ======================================
void TIM_PWM_Led1_set_val( uint16_t ccp )
{
 TIM_PWM->TIM_PWM_LED1_CH = ccp;
}
void TIM_PWM_Led0_set_val( uint16_t ccp )
{
 TIM_PWM->TIM_PWM_LED0_CH = ccp;
}
...
void TIM_PWMOut_Init( uint16_t arr, uint16_t psc, uint16_t ccp )
{
    ...
    GPIO_Init( PWM_LED0_GPIO, &GPIO_InitStructure );

    GPIO_InitStructure.GPIO_Pin = PWM_LED1_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init( PWM_LED1_GPIO, &GPIO_InitStructure );
...
void gpio_init(void)
{
   ...
   ////============= PWM0_LED ==================================
    ...
    GPIO_Init(PWM_LED0_GPIO, &GPIO_InitStructure);
    GPIO_ResetBits(PWM_LED0_GPIO, PWM_LED_PIN);
#endif
    ////============= ON_DV ==================================
...
///=============== led pwm =======================================
void set_led0_pwm(uint16_t led_pwm)
{
    TIM_PWM_Led0_set_val(led_pwm);
}
void set_led1_pwm(uint16_t led_pwm)
{
