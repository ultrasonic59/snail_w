...
void SRAM_WriteBuffer(uint16_t* pBuffer, uint32_t WriteAddr, uint32_t NumHalfwordToWrite)
{
  for (; NumHalfwordToWrite != 0; NumHalfwordToWrite--) /* while there is data to write */
  {
    ...
    *(uint32_t *) (Bank1_SRAM3_ADDR + WriteAddr) = *pBuffer++;
...
void SRAM_ReadBuffer(uint16_t* pBuffer, uint32_t ReadAddr, uint32_t NumHalfwordToRead)
{
  for (; NumHalfwordToRead != 0; NumHalfwordToRead--) /* while there is data to read */
  {
    ...
    *pBuffer++ = *(__IO uint32_t*) (Bank1_SRAM3_ADDR + ReadAddr);
...
void SRAM_TEST(void)
{
  ...
  for(i=0;i<32;i++)
  {		  
	  Fill_Buffer(TxBuffer, BUFFER_SIZE, 0x32f);
	  Fill_RxBuffer(RxBuffer, BUFFER_SIZE, 0x0);
	  SRAM_WriteBuffer(TxBuffer, WRITE_READ_ADDR+(BUFFER_SIZE*i), BUFFER_SIZE);
	  //printf("\r\nSRAM has Write..\r\n");
	  /* Read data from FSMC SRAM memory */
	  SRAM_ReadBuffer(RxBuffer, WRITE_READ_ADDR+(BUFFER_SIZE*i), BUFFER_SIZE);  
	  //printf("\r\nSRAM has Read..\r\n");
	  //printf("\r\nSRAM Read Data:");
...
void Fill_RxBuffer(uint16_t *pBuffer, uint16_t BufferLenght, uint32_t Offset)
{
  uint32_t IndexTmp = 0;
